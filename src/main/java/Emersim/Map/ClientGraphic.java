...
public class ClientGraphic implements Graphic {
    ...
    */
    ...
    ClientGraphic(LatLng latLng, GuiComponents guiComponents) {
        ...
        areaPeri = new Polyline(MapConfig.map);
        ...
        marker = new Marker(MapConfig.map);
    ...
    ClientGraphic(LinkedList<LatLng> path, GuiComponents guiComponents) {
        ...
        MapConfig.clientGraphics.add(this);
    }
    ...
    private void finaliseArea() {
        if (path.size() < 3) {
            ...
            // Store area for use in simulation
            MapConfig.clientGraphics.add(this);
            MapConfig.areaBeingDrawn = null;
            MapConfig.buttonState = MapConfig.BUTTON_STATE.NONE;
    ...
    private void addEventListeners() {
        polygon.addEventListener("click", new MapMouseEvent() {
            @Override
            public void onEvent(MouseEvent mouseEvent) {
                infoWindow.open(MapConfig.map, polygon.getPaths()[0][0]);
            }
        });
    ...
    private void generateInfoWindow() {
        // Creating an information window
        infoWindow = new InfoWindow(MapConfig.map);
        // Putting the address and location to the content of the information window
        infoWindow.setContent("<b>Client region #" + MapConfig.clientGraphics.size() + "</b>");
    ...
    private void generatePolygon() {
        polygon = new Polygon(MapConfig.map);
    ...
    public void remove() {
        ...
        infoWindow.close();
        MapConfig.clientGraphics.remove(this);
